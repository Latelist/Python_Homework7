...
n = int(input('Сколько кустиков: '))
lst = [randint(1,10) for i in range(n)]
print(lst)
max_sum = lst[0] + lst[1] + lst[-1]
for i in range(1, n):
    if i == n-1:
        if lst[i] + lst[i-1] + lst[0] > max_sum:
            max_sum = lst[i] + lst[i-1] + lst[0]
    else:
        if lst[i] + lst[i-1] + lst[i+1] > max_sum:
            max_sum = lst[i] + lst[i-1] + lst[i+1]
